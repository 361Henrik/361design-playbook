...
const GuidelinesPage = () => {
  return (
    <div className="px-8 py-10 max-w-5xl">
      ...
      {/* Narrative Path — Graphic Principles */}
      <div className="mt-16">
        <h2 className="font-display text-h2 text-foreground mb-2">Narrative Path</h2>
        <p className="text-body font-body text-muted-foreground max-w-prose mb-8">
          A graphic pattern for representing progression — use it for storytelling, explanations, journeys, and onboarding.
        </p>

        {/* Purpose */}
        <div className="mb-10">
          <h3 className="font-display text-h3 text-foreground mb-4">Purpose</h3>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
            {[
              ...
              <div key={item.label} className="p-4 rounded-md border border-border bg-card">
                ...
                <p className="text-label font-body font-medium text-foreground mb-1">{item.label}</p>
                <p className="text-caption font-body text-muted-foreground">{item.desc}</p>
              </div>
            ))}
          </div>
        </div>
        ...
        {/* Graphic Principles */}
        <div className="mb-10">
          <h3 className="font-display text-h3 text-foreground mb-4">Graphic Principles</h3>

          {/* Principle 1: Flowing path */}
          <div className="p-5 rounded-md border border-border bg-card mb-4">
            <p className="text-label font-body font-medium text-foreground mb-1">Flowing path lines</p>
            <p className="text-body-sm font-body text-muted-foreground mb-4">
                ...
                {["Arrive", "Discover", "Reflect"].map((label, i, arr) => (
                  <div key={label} className="flex gap-3">
                    <div className="flex flex-col items-center">
                      <div className="w-6 h-6 rounded-full bg-primary text-primary-foreground flex items-center justify-center text-caption font-body font-medium shrink-0">
                        {i + 1}
                      </div>
                      {i < arr.length - 1 && <div className="w-px flex-1 bg-border min-h-[20px]" />}
                    </div>
                    <p className="text-body-sm font-body text-foreground pb-5">{label}</p>
                  </div>
                ))}
              </div>
            </div>
          ...
          {/* Principle 2: Modular information */}
          <div className="p-5 rounded-md border border-border bg-card mb-4">
            <p className="text-label font-body font-medium text-foreground mb-1">Modular information</p>
            <p className="text-body-sm font-body text-muted-foreground mb-4">
              ...
              <div className="grid grid-cols-3 gap-4">
                {[
                  ...
                  <div key={mod.title} className="p-4 rounded-md border border-border bg-background">
                    ...
                    <p className="text-label font-body font-medium text-foreground mb-1">{mod.title}</p>
                    <p className="text-caption font-body text-muted-foreground">{mod.desc}</p>
                  </div>
                ))}
              </div>
            </div>
          ...
          {/* Principle 3: Spacing rules */}
          <div className="p-5 rounded-md border border-border bg-card">
            <p className="text-label font-body font-medium text-foreground mb-1">Spacing &amp; density</p>
        ...
        {/* Do / Don't */}
        <div>
          <h3 className="font-display text-h3 text-foreground mb-4">Do / Don't</h3>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div className="p-4 rounded-md border border-primary/20 bg-primary/5">
              <p className="text-label font-body font-medium text-primary mb-2">Do</p>
              <ul className="space-y-1.5 text-body-sm font-body text-muted-foreground">
            ...
            <div className="p-4 rounded-md border border-destructive/20 bg-destructive/5">
              <p className="text-label font-body font-medium text-destructive mb-2">Don't</p>
              <ul className="space-y-1.5 text-body-sm font-body text-muted-foreground">
