    ...
    <div className="px-space-5 md:px-space-8 py-space-8 max-w-content">
      ...
      <Tabs defaultValue="rules" className="mt-6">
            ...
            <div className="p-5 rounded-md border border-border bg-card">
              ...
              <div className="grid grid-cols-2 sm:grid-cols-3 gap-3 mt-3">
                {["color", "background-color", "border-color", "opacity", "box-shadow", "transform (translate only)"].map((prop) => (
                  <div key={prop} className="px-3 py-2 rounded bg-primary/5 border border-primary/10 text-xs font-body text-card-foreground">{prop}</div>
                ))}
              </div>
            </div>
        ...
        <TabsContent value="examples">
          <TokenExamplesTab tokenCategory="motion" />
        </TabsContent>
      </Tabs>
    </div>
  );
