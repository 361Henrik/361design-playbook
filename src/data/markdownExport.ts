...
export function generateAIContextFile(): string {
  ...
  lines.push("4. **Use the specified typefaces.** Display: Playfair Display (font-display). Body: Lexend (font-body). Mono: monospace (font-mono).");
  ...
  lines.push("10. **Surface target rule.** Helmut requires 48px targets and 52px map hit areas; Olga requires 44px controls. Keep generous spacing, high-contrast text, and clear hierarchy.");
  lines.push("");
  lines.push("---");
  lines.push("");
